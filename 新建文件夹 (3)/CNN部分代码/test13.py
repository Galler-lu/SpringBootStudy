...
for i in range(len(xy)):
    ...
    # 读取acc数据
    temp_list_acc.append([list(xy[i, 18:138])])

x_data = torch.from_numpy(np.array(temp_list))

x_data_acc = torch.from_numpy(np.array(temp_list_acc))
...
class NetModel1(torch.nn.Module):
    def __init__(self):
        ...
        self.conv_ghwa_3 = torch.nn.Conv1d(1, 16, kernel_size=3, padding=1)
        self.conv_ghwa_4 = torch.nn.Conv1d(16, 32, kernel_size=3, padding=1)
        self.relu = torch.nn.ReLU()
        self.averagePooling1 = torch.nn.AvgPool1d(6)
        self.averagePooling2 = torch.nn.AvgPool1d(20)
        self.linear = torch.nn.Linear(32, 1)
        self.softmax = torch.nn.Softmax(dim=1)
        self.sigmoid = torch.nn.Sigmoid()
        self.linearTotal = torch.nn.Linear(64, 1)
    ...
    def forward(self, x, x_acc):
        ...
        x_acc = self.relu(self.conv_ghwa_3(x_acc))
        x_acc = self.averagePooling1(x_acc)
        x_acc = self.relu(self.conv_ghwa_4(x_acc))
        x_acc = self.averagePooling2(x_acc)
        x_acc = x_acc.view(-1, 32)

        x_total = torch.cat([x, x_acc], 1)
        print('x_total的size为：',x_total.size())

        # return self.sigmoid(self.linear(x))
        return self.sigmoid(self.linearTotal(x_total))
...
for i in range(len(xy1)):
    ...
    # 读取acc数据
    temp_list1_acc.append([list(xy[i, 18:138])])

x_data1 = torch.from_numpy(np.array(temp_list1))
x_data1_acc = torch.from_numpy(np.array(temp_list1_acc))
...
def test():
    ...
    correct1 = 0
    correct0 = 0
    outputs = model(x_data1.float(), x_data1_acc.float())
...
def train(number):
    ...
    for epoch in range(number):
        y_pred = model(x_data.float(), x_data_acc.float())
